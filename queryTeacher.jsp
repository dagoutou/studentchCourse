...
<div class="container">
		<div class="row">
			<div class="col-md-12">
			<h5>教师管理</h5>
			<form name="studentquery" action="/Coursesel/AdminHandler/queryTea/1" method="get">
				<div class="row">
					<div class="col-md-4">
						...
						<select name="serc" class="form-control select select-primary select-block mbl">
							...
							<option value="tid">按教师工号</option>
							<option value="tname">按教师姓名</option>
						</select>
					</div>
					<div class="col-md-4">
			...
			<c:forEach var="teacher" items="${requestScope.teacherList}">
				<tr>
					...
					<td><a class="btn btn-default" href="/Coursesel/AdminHandler/modityTea/${teacher.tid}">
							修改 </a></td>

					<td><a class="deleteCss btn btn-default"
						href="/Coursesel/AdminHandler/deleteTea/${teacher.tid}"> 删除 </a></td>
				</tr>
	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			<ul class="pagination bg-primary">
				<li><a href="/Coursesel/AdminHandler/queryTea/1?serc=${requestScope.serc}&condition=${requestScope.condition}">首页</a></li>
				...
				<c:forEach items="${pageInfo.navigatepageNums }" var="page_Num">
					...
					<c:if test="${page_Num != pageInfo.pageNum }">
						<li><a href="/Coursesel/AdminHandler/queryTea/${page_Num }?serc=${requestScope.serc}&condition=${requestScope.condition}">${page_Num }</a></li>
					</c:if>
 
				</c:forEach>
				<c:if test="${pageInfo.hasNextPage }">
					<li><a href="/Coursesel/AdminHandler/queryTea/${pageInfo.pageNum+1}?serc=${requestScope.serc}&condition=${requestScope.condition}"
						aria-label="Next"> <span aria-hidden="true">»</span>
					</a></li>
				</c:if>
				<li><a href="/Coursesel/AdminHandler/queryTea/${pageInfo.pages}?serc=${requestScope.serc}&condition=${requestScope.condition}">末页</a></li>
			</ul>
			</nav>
		</div>
	</div>
